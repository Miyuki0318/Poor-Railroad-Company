...
namespace basecross {
	...
	struct CONTROLER_STATE : public XINPUT_STATE
	{
		...
		BYTE bCurrentLeftTrigger;	///< 左トリガ
		BYTE bRightTrigger;	///< 右トリガ
		BYTE bCurrentRightTrigger;	///< 右トリガ
		bool wPressedLTrigger;
		bool wPressedRTrigger;
		bool wReleasedLTrigger;
		bool wReleasedRTrigger;
	...
	public:
		...
		void ResetControlerState() {
			...
			for (DWORD i = 0; i < MAX_CONTROLLERS; i++)
			{
				...
				if (dwResult == ERROR_SUCCESS) {
					...
					if (m_State[i].bCurrentRightTrigger > 0 && m_State[i].bLeftTrigger == 0)
					{
						m_State[i].wReleasedLTrigger = true;
					}
					else
					{
						m_State[i].wReleasedLTrigger = false;
					}

					if (m_State[i].bCurrentRightTrigger > 0 && m_State[i].bRightTrigger == 0)
					{
						m_State[i].wReleasedRTrigger = true;
					}
					else
					{
						m_State[i].wReleasedRTrigger = false;
					}

					if (m_State[i].bLeftTrigger > 0 && m_State[i].bCurrentLeftTrigger <= 0)
					{
						m_State[i].wPressedLTrigger = true;
					}
					else
					{
						m_State[i].wPressedLTrigger = false;
					}
					m_State[i].bCurrentLeftTrigger = m_State[i].bLeftTrigger;

					if (m_State[i].bRightTrigger > 0 && m_State[i].bCurrentRightTrigger <= 0)
					{
						m_State[i].wPressedRTrigger = true;
					}
					else
					{
						m_State[i].wPressedRTrigger = false;
					}
					m_State[i].bCurrentRightTrigger = m_State[i].bRightTrigger;
		...
		*/
		//--------------------------------------------------------------------------------------
		const vector<CONTROLER_STATE>& GetControllerVec()const { return m_State; }
	};
...
*/
	...
	public:
		...
		*/
		...
		shared_ptr<TextureResource> RegisterTexture(const wstring& Key,
			const wstring& TextureFileName, bool b, const wstring& TexType = L"WIC");
		//--------------------------------------------------------------------------------------
		/*!
